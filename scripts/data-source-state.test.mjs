...
test('App activates source snapshots and loads legacy point values only when referenced', () => {
  ...
  assert.match(appSource, /await pointCatalogGateway\.activateWorkspace\(targetWorkspace\)[\s\S]*?sourceBindingRuntime\.reset\?\.\(\{ keepBindings: true \}\)[\s\S]*?await refreshPointCatalog\(options\)[\s\S]*?await replaySourceSnapshotsForNodes\(\)/)
  ...
  assert.match(appSource, /onMounted\(async \(\) => \{[\s\S]*?try \{\s*await activatePointCatalogWorkspace[\s\S]*?catch \(error\) \{[\s\S]*?数据源暂不可用[\s\S]*?let restored = await restoreWorkspacePaperSessions/)
  assert.match(appSource, /async function switchWorkspace\(\)[\s\S]*?catch \(error\) \{[\s\S]*?await activatePointCatalogWorkspace\(workspaceId\.value\)[\s\S]*?已保留当前图纸/)
})
