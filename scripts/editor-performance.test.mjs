...
test('node memoization keeps runtime subscriptions out of the parent render loops', async () => {
  ...
  assert.match(appSource, /<MiniMapPreview v-if="previewFitMounted"[^>]*:runtime-store="runtimeData"[^>]*:render-budget-ms="previewFitActive \? 4 : 2"[^>]*:render-mode="previewFitActive \? 'task' : 'idle'"[^>]*faithful[^>]*@render-complete="handlePreviewFitRenderComplete"/)
  ...
  assert.match(appSource, /const previewDomDensity = computed\([\s\S]*?PREVIEW_DOM_EDGE_LIMIT[\s\S]*?PREVIEW_DOM_DRAWING_LIMIT/)
  ...
  assert.match(previewLayerSource, /const visibleNodes = shallowRef\(\[\]\)/)
  assert.match(previewLayerSource, /partitionRetainedPreviewNodes\(source, visibleNodes\.value\)[\s\S]*?visibleNodes\.value = retainedNodes/)
  assert.match(previewLayerSource, /visibleNodes\.value\.push\(\.\.\.pendingNodes\.slice\(pendingCount, nextCount\)\)[\s\S]*?triggerRef\(visibleNodes\)/)
  assert.doesNotMatch(previewLayerSource, /v-for="batch in batches"/)
